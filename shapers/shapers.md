...
1.  When we want to describe the specific numbers shown on dice, we will write them as `1`, `2`, and so on up to `6`. When we talk about other numbers, such as the total score rolled on three dice, we will write them as 7, 11, and so on.
...
3.  Remember that adding a negative number is the same as subtracting a positive one. For example, if someone has a bonus of +3 on a roll and a penalty of -4, their actual roll is 3D+3-4 or 3D-1, i.e., the total on three dice *minus* one point.

## How to Play

Each game has one or more [**players**](#g:player) and a [**Director**](#g:director). Each player takes on the role of one [**character**](#g:character), while the Director plays all the other characters, who are called [**extras**](#g:extra). The Director can be an adult, but children age 11 and older can also be Directors.
...
Every character has strengths and weaknesses that determine what they're good at and what they aren't. We describe this by giving every character a [**rating**](#g:rating) for five [**features**](#g:feature):
...
So what is a rating? It's simply a number: 0 if the character is average, positive if they're higher than average, and negative if they're lower. A positive rating is sometimes called a [**bonus**](#g:bonus), and a negative one is sometimes called a [**penalty**](#g:penalty).
...
A [**challenge**](#g:challenge) is something a character might want to do, like climb a wall or pick a lock. Every challenge has a rating as well: difficult challenges have negative ratings (penalties), while easy challenges have positive ones (bonuses).
...
If the total adjustment for a challenge (i.e., the character's rating plus the challenge's difficulty) is positive, and the outcome isn't critical to the game, the Director can decide that the characer gets an [**automatic success**](#g:automatic-success). This rule helps keep the game moving.
...
A situation in which one character is trying to out-do another is called a [**contest**](#g:contest). In this case, both players roll dice and add their ratings, and whoever has the higher score wins. If there is a tie, the Director rules on whether the contest continues or whether something unusual happened.
...
1.  If an attack is a complete surprise, the defender doesn't get a defense roll. (The attacker can still miss, though—it's embarrassing.)
...
Every living character belongs to a [**species**](#g:species). They all stand upright, speak, and have hands, but they also have fur (or feathers) and their feet look like animals' feet. Different species have different default values for the seven features, which are shown below.
...
Crows are fast-talking deal-makers. Their feathered appendages make them somewhat clumsy, and they are fascinated by shiny objects. Crows cannot fly by default but can learn how to do it over short distances.
...
The only birds still capable of flying well, parrots are also gifted mimics.
...
-   Their sense of smell is +2, not -2.
...
As proud as eagles, tigers are scornful of creatures that live in herds or packs, especially lions.
...
Slow-moving, short-sighted, and natural shapers.
...
-   Can hold their breath for 15 minutes.
-   If a Sloth is a shaper, they start with a -1 rating instead of -4.
...
Players can get extra points to distribute by *lowering* ratings as well: they may lower any rating by 1 or 2 points and use those points to improve other skills. They cannot lower any rating by more than 3 points, and still cannot raise any rating by more than 3 points.
...
Every character has [**skills**](#g:skill). Some are automatic: for example, almost everyone can try to throw a rock. Others are not: nobody can play the oboe if they've never seen one before.

Just like features, skills have ratings. Positive ratings mean that characters are better than average, and negative ratings mean that they find even simple tasks difficult. Some common skills are listed below, along with a character's initial rating:
...
| Weapon    | FIT - 2    | sword, spear, pistol, bow, etc.           |

### Choosing Skills

When a player is creating a character, she can spend (2xINT)+6 points improving the character's skills. For example, if a character has an INT of +3, they have 12 points to spend on raising skills. If their INT is -2, on the other hand, they only have 2 points to spend on skills (they're a slower learner).
...
A small number of animal characters have psychic powers that let them control or [**shape**](#g:shaping) the world with their mind. Characters who can shape typically start with a rating of -4, which improves with practice and training. The six known kinds of shaping are:
